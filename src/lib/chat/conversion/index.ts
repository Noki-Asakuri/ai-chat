...
export type MyUIMessage = UIMessage<z.infer<typeof metadataSchema>, {}, UITools>;
...
function convertDataMessage(msg: V4Message, index: number): MyUIMessage {
  return { ...createBaseMessage(msg, index), parts: [] };
}
...
function buildPartsFromTopLevelFields(msg: V4Message): MyUIMessage["parts"] {
  ...
  if (msg.experimental_attachments) {
    msg.experimental_attachments.forEach((a) => {
      parts.push({ type: "file", mediaType: a.contentType!, url: a.url, filename: a.name });
    });
  }
...
export function convertV4MessageToV5(msg: V4Message, index: number): MyUIMessage {
  // if (!isV4Message(msg)) {
  ...
  for (const part of msg.parts) {
    switch (true) {
      ...
      case part.type === "source-url": {
        parts.push({
          type: "source",
          source: { id: part.sourceId, url: part.url, title: part.title, sourceType: "url" },
        });
        break;
      }

      case part.type === "file": {
        parts.push({ type: "file", mimeType: part.mediaType, data: part.url });
        break;
      }

      // For custom data, but we don't have any
      // case "data-custom": {
      //   base.data = part.data;
      //   break;
      // }
