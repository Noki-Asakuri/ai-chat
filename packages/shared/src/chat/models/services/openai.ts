import type { ModelData, ModelIdKey } from "..";

export const openai: Record<ModelIdKey, ModelData> = {
  "openai/gpt-4.1": {
    display: { name: "GPT 4.1" },
    id: "openai/gpt-4.1",
    provider: "openai",
    runtime: { api: "chat" },
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: { toolCalling: true },
  },

  "openai/chatgpt-4o": {
    display: { name: "ChatGPT 4o" },
    id: "openai/chatgpt-4o",
    provider: "openai",
    runtime: { api: "chat", modelId: "chatgpt-4o-latest" },
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {},
    deprecation: {
      message: "ChatGPT 4o is deprecated and can no longer be used. Please switch to GPT 5.2.",
      replacementModelId: "openai/gpt-5.2",
    },
  },
  "openai/gpt-4o": {
    display: { name: "GPT 4o" },
    id: "openai/gpt-4o",
    provider: "openai",
    runtime: { api: "chat" },
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {},
    deprecation: {
      message: "GPT 4o is deprecated and can no longer be used. Please switch to GPT 5.2.",
      replacementModelId: "openai/gpt-5.2",
    },
  },

  "openai/o3": {
    display: { name: "o3" },
    id: "openai/o3",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },

  "openai/o3-mini": {
    display: { name: "o3 Mini" },
    id: "openai/o3-mini",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },
  "openai/o4-mini": {
    display: { name: "o4 Mini" },
    id: "openai/o4-mini",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },

  "openai/gpt-5": {
    display: { name: "GPT 5" },
    id: "openai/gpt-5",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      imageGeneration: true,
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },
  "openai/gpt-5-mini": {
    display: { name: "GPT 5 Mini" },
    id: "openai/gpt-5-mini",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      imageGeneration: true,
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },
  "openai/gpt-5-nano": {
    display: { name: "GPT 5 Nano" },
    id: "openai/gpt-5-nano",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },
  "openai/gpt-5-chat": {
    display: { name: "GPT 5 Chat" },
    id: "openai/gpt-5-chat",
    altModelIds: ["openai/gpt-5-chat-latest"],
    provider: "openai",
    runtime: { modelId: "gpt-5-chat-latest" },
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
    },
  },
  "openai/gpt-5-codex": {
    display: { name: "GPT 5 Codex" },
    id: "openai/gpt-5-codex",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },
  "openai/gpt-5-pro": {
    display: { name: "GPT 5 Pro" },
    id: "openai/gpt-5-pro",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },

  "openai/gpt-5.1": {
    display: { name: "GPT 5.1" },
    id: "openai/gpt-5.1",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },

  "openai/gpt-5.1-chat": {
    display: { name: "GPT 5.1 Chat" },
    id: "openai/gpt-5.1-chat",
    provider: "openai",
    runtime: { modelId: "gpt-5.1-chat-latest" },
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
    },
  },

  "openai/gpt-5.1-codex": {
    display: { name: "GPT 5.1 Codex" },
    id: "openai/gpt-5.1-codex",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },

  "openai/gpt-5.1-codex-mini": {
    display: { name: "GPT 5.1 Codex Mini" },
    id: "openai/gpt-5.1-codex-mini",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },

  "openai/gpt-5.2": {
    display: { name: "GPT 5.2" },
    id: "openai/gpt-5.2",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },
  "openai/gpt-5.2-chat": {
    display: { name: "GPT 5.2 Chat" },
    id: "openai/gpt-5.2-chat",
    provider: "openai",
    runtime: { modelId: "gpt-5.2-chat-latest" },
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
    },
  },
  "openai/gpt-5.2-pro": {
    display: { name: "GPT 5.2 Pro" },
    id: "openai/gpt-5.2-pro",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },
  "openai/gpt-5.2-codex": {
    display: { name: "GPT 5.2 Codex" },
    id: "openai/gpt-5.2-codex",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },

  "openai/gpt-5.3-chat": {
    display: { name: "GPT 5.3 Chat" },
    id: "openai/gpt-5.3-chat",
    provider: "openai",
    runtime: { modelId: "gpt-5.3-chat-latest" },
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
    },
  },
  "openai/gpt-5.3-codex": {
    display: { name: "GPT 5.3 Codex" },
    id: "openai/gpt-5.3-codex",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },

  "openai/gpt-5.4": {
    display: { name: "GPT 5.4" },
    id: "openai/gpt-5.4",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },
  "openai/gpt-5.4-pro": {
    display: { name: "GPT 5.4 Pro" },
    id: "openai/gpt-5.4-pro",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },
  "openai/gpt-5.4-mini": {
    display: { name: "GPT 5.4 Mini" },
    id: "openai/gpt-5.4-mini",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },
  "openai/gpt-5.4-nano": {
    display: { name: "GPT 5.4 Nano" },
    id: "openai/gpt-5.4-nano",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },

  "openai/gpt-5.5": {
    display: { name: "GPT 5.5" },
    id: "openai/gpt-5.5",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },
  "openai/gpt-5.5-pro": {
    display: { name: "GPT 5.5 Pro" },
    id: "openai/gpt-5.5-pro",
    provider: "openai",
    modalities: { input: ["image", "pdf", "text"], output: ["text"] },
    capabilities: {
      toolCalling: true,
      reasoning: { type: "selectable", defaultLevel: "medium", levels: ["low", "medium", "high"] },
    },
  },
};
