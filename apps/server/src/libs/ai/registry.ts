import { type ModelIdKey, ModelsData } from "@ai-chat/shared/chat/models";
...
export function createModelRegistry(options: CreateModelRegistryOptions) {
  ...
  const openai = customProvider({
    imageModels: {
      "gpt-image-1.5": baseOpenai.imageModel("gpt-image-1.5"),
      "gpt-image-2": baseOpenai.imageModel("gpt-image-2"),
    },
    ...
    languageModels: {
      ...
      "gpt-5.5": baseOpenai.languageModel("gpt-5.5"),
      "gpt-5.5-pro": baseOpenai.languageModel("gpt-5.5-pro"),
    },
  });
  ...
  const kimi = customProvider({
    languageModels: {
      ...
      "kimi-k2.6": baseKimi.languageModel("kimi-k2.6"),
    },
  });
  ...
  const registry: typeof providers.languageModel = (...args) => {
    ...
    return wrapLanguageModel({ model: providers.languageModel(...args), middleware });
  };
