...
const baseToastManager = ToastPrimitive.createToastManager<ToastData>();

type ToastOptions = Omit<ToastManagerAddOptions<ToastData>, "title" | "type">;

function addToast(type: string | undefined, title: ReactNode, options?: ToastOptions): string {
  return baseToastManager.add({ type, title, ...options });
}

const toast = {
  add: baseToastManager.add,
  close: baseToastManager.close,
  update: baseToastManager.update,
  ...
  message(title: ReactNode, options?: ToastOptions) {
    return addToast(undefined, title, options);
  },
};
...
function Toast({ className, ...props }: ToastPrimitive.Root.Props) {
  return (
    <ToastPrimitive.Root
      data-slot="toast"
      className={cn(
        "group/toast pointer-events-auto absolute inset-x-0 top-0 z-[calc(1000-var(--toast-index))] mx-auto w-full origin-top border bg-popover text-popover-foreground shadow-lg will-change-transform outline-none select-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50",
...
function Toaster({ children, toastManager = baseToastManager, ...props }: ToastPrimitive.Provider.Props) {
  return (
